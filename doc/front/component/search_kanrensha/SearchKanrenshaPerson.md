# コンポーネント設計書: SearchKanrenshaPerson.vue

## 1. 概要

- 関連者（個人）マスタを前方一致検索し、一覧から特定のデータを選択するための検索コンポーネントです。
- 前方一致検索条件として「名前」「住所」「職業」の入力フォームを提供します。
- 検索実行時、セッション管理されたJWT認証トークンを安全に取得し、バックエンドAPIへ `POST` リクエストを行います。詳細なAPI仕様は、[使用API設計書: user-kanrensha/search-person](../../use_api/searchPerson.md) を参照してください。
- 検索結果の一覧は、グリッド（テーブル）形式で出力し、自作の `PagingControl` コンポーネントにより1ページあたり20件の標準的なページング操作に対応しています。
- 一覧の行から「選択」が指示された場合、対応する個人エンティティ（マスタレコード）を Emits を経由して親コンポーネントに送信します。
...
![画面イメージ](./SearchKanrenshaPerson_1.png "法人番号検索画面イメージ")
![画面イメージ](./SearchKanrenshaPerson_2.png "法人番号画面イメージ")
...
|      イベント名       |                          引数                          |                         説明                         |
| :-------------------- | :----------------------------------------------------- | :--------------------------------------------------- |
| `sendPersonInterface` | `selectedEntity: KanrenshaPersonMasterEntityInterface` | グリッドで選択された個人エンティティデータを送信。   |
| `sendCancelPerson`    | なし                                                   | 検索を終了し、コンポーネントを閉じる指示を親に通知。 |

## 5. データ構造 (DTO)

コンポーネント内外の検索および結果処理で以下の DTO および Entity インターフェースを使用します。

### `SearchKanrenshaPersonCapsuleDtoInterface` 仕様
...
| `shokugyou`  | `string` | 制限無 | 職業（前方一致）                        |
| `allCount`   | `number` |   -    | 総件数（ページング共通）                |
| `limit`      | `number` |   -    | 1ページあたりの件数（ページング共通）   |
| `pageNumber` | `number` |   -    | 現在のページ番号（ページング共通、0〜） |

### `SearchKanrenshaPersonResultDtoInterface` 仕様

※ `FrameworkPagingDtoInterface` のプロパティを継承します。

|    プロパティ名    |                    型                    |              説明              |
| :----------------- | :--------------------------------------- | :----------------------------- |
| `listMasterPerson` | `KanrenshaPersonMasterEntityInterface[]` | 関連者個人マスタ検索結果リスト |
| `allCount`         | `number`                                 | 総件数                         |
| `limit`            | `number`                                 | 1ページあたりの件数            |
| `pageNumber`       | `number`                                 | 現在のページ番号               |

### `KanrenshaPersonMasterEntityInterface` 仕様

|       プロパティ名        |    型    |                      説明                      |
| :------------------------ | :------- | :--------------------------------------------- |
| `kanrenshaPersonMasterId` | `number` | テーブル主キーId                               |
| `personKanrenshaCode`     | `string` | 個人関連者コード                               |
| `kanrenshaName`           | `string` | 個人名                                         |
| `allAddress`              | `string` | 個人全住所                                     |
| `personShokugyou`         | `string` | 個人職業                                       |
...
- **処理内容**：
  ...
  2. [トークン取得共通関数: getAuthorizedPromiseArea()](../../use_api/getAuthorizedPromiseArea.md) を呼び出して認証トークンを取得します。
     - **トークン取得成功時**：
       - バックエンドの [使用API設計書: user-kanrensha/search-person](../../use_api/searchPerson.md) に対して `POST` リクエストを送信します。
         - メソッド：`POST`
       ...
       - **API接続成功時 (200 OK等)**：
         - レスポンスのJSONデータを `resultDto.value`（`SearchKanrenshaPersonResultDtoInterface`）に代入します。
         - 取得したマスタレコードリスト（`resultDto.value.listMasterPerson`）の長さが `0`（件数ゼロ）の場合：
...
- **引数**：`selectedNo: number`（選択されたレコードの `kanrenshaPersonMasterId`）
- **処理内容**：
  1. `resultDto.value.listMasterPerson` 配列をフィルターし、主キー `kanrenshaPersonMasterId` が `selectedNo` に一致するエンティティオブジェクト（`selectedEntity`）を取得します。
  2. 該当オブジェクトが存在する場合、`sendPersonInterface` イベントを発火し、引数に `selectedEntity` を渡して親コンポーネントに通知します。
  3. 取得できなかった場合、JavaScriptの `alert("取得できませんでした")` を呼び出します。

### 6.3 `onCancel()`

- **契機**：「キャンセル」ボタン押下時（`isRaiseCommponet` が有効な場合）
- **処理内容**：
  1. `sendCancelPerson` イベントを発火して親コンポーネントに通知します。
