<script setup lang="ts">
import { ref, type Ref } from 'vue';
import RoutePathConstants from '../../../../routePathConstants';
import { AccessTokenNotFoundError, TokenRefreshError } from '../../dto/login/errors';
import getAuthorizedPromiseArea from '../../dto/login/getAuthorizedPromiseArea';
import { SearchKanrenshaKigyouDtCapsuleDto, type SearchKanrenshaKigyouDtCapsuleDtoInterface } from '../../dto/kanrensha/searchKanrenshaKigyouDtCapsuleDto';
import { SearchKanrenshaKigyouDtResultDto, type SearchKanrenshaKigyouDtResultDtoInterface } from '../../dto/kanrensha/searchKanrenshaKigyouDtResultDto';
import type { KanrenshaKigyouDtMasterEntityInterface } from '../../entity/kanrenshaKigyouDtMasterEntity';
import { MessageConstants } from '../../dto/message/messageConstants';
import PagingControl from '../paging/PagingControl.vue';
import MessageView from '../message/MessageView.vue';
import { getErrorMessage, getErrorUniqueIdMessage } from '../../dto/errorFunction.ts';

//props,emit
const props = defineProps<{ isRaiseCommponet: boolean }>();
const emits = defineEmits(["sendKigyouDtInterface", "sendCancelKigyouDt"]);

// よく使う定数
const BLANK: string = "";
const INIT_NUMBER: number = 0;
// const INIT_BOOLEAN: boolean = false;
// const SERVER_STATUS_OK: number = 200;
// const SERVER_STATUS_ERROR: number = 400;
const SEARCH_LIMIT: number = 20;
const INQUIRE_FLG: boolean = false;
const ERR_MESS_ONLY: boolean = true;
const MESS_PAGE_NAME: string = "関連者企業・団体検索モーダル";
const INIT_CALLER: string = "no branch";

// メッセージボックス表示定数
const infoLevel: Ref<number> = ref(MessageConstants.LEVEL_NONE);
const messageType: Ref<number> = ref(MessageConstants.VIEW_NONE);
const caller: Ref<string> = ref(INIT_CALLER);
const message: Ref<string> = ref(BLANK);

// Paging
const pageNumber: Ref<number> = ref(INIT_NUMBER);
const allCount: Ref<number> = ref(INIT_NUMBER);
const limit: Ref<number> = ref(SEARCH_LIMIT);

// back側アクセス
const urlBack: string = RoutePathConstants.DOMAIN + RoutePathConstants.BASE_PATH;

// 検索
const capsuleDto: Ref<SearchKanrenshaKigyouDtCapsuleDtoInterface> = ref(new SearchKanrenshaKigyouDtCapsuleDto());
const resultDto: Ref<SearchKanrenshaKigyouDtResultDtoInterface> = ref(new SearchKanrenshaKigyouDtResultDto());

// 検索リスト
function onSearch() {

    capsuleDto.value.allCount = allCount.value;
    capsuleDto.value.limit = limit.value;
    capsuleDto.value.pageNumber = pageNumber.value;

    getAuthorizedPromiseArea().then(token => {
        const url = urlBack + "/user-kanrensha/search-kigyou-dt";
        const method = "POST";
        const body = JSON.stringify(capsuleDto.value);
        const headers = {
            'Accept': 'application/json',
            'Content-Type': 'application/json',
            'X-AUTH-TOKEN': 'Bearer ' + token
        };
        fetch(url, { method, headers, body })
            .then(async (response) => {
                resultDto.value = await response.json();
                if (0 == resultDto.value.listMasterKigyouDt.length) {
                    infoLevel.value = MessageConstants.LEVEL_INFO;
                    messageType.value = MessageConstants.VIEW_TOAST;
                    caller.value = "onSea";
                    message.value = "検索結果が0件でした";
                }
                allCount.value = resultDto.value.allCount;
                limit.value = resultDto.value.limit;
                pageNumber.value = resultDto.value.pageNumber;
            })
            .catch((error) => {
                infoLevel.value = MessageConstants.LEVEL_ERROR;
                messageType.value = MessageConstants.VIEW_OK;
                //title.value = MESS_PAGE_NAME;
                message.value = getErrorMessage(error, ERR_MESS_ONLY);
            });
    }).catch((e) => {
        infoLevel.value = MessageConstants.LEVEL_ERROR;
        messageType.value = MessageConstants.VIEW_OK;

        if (e instanceof AccessTokenNotFoundError) {
            // トークン保持ができていない場合
            //title.value = "現在トークンが存在しません";
            message.value = e.message;
            return;
        }
        if (e instanceof TokenRefreshError) {
            // 取得に失敗している場合
            //title.value = "有効期限まじかのトークンを再取得できませんでした";
            message.value = e.message;
            return;
        }
        //title.value = MESS_PAGE_NAME;
        message.value = getErrorMessage(e, INQUIRE_FLG);
        return;
    });

}

/** 行選択 */
function onSelectRow(selectedNo: number) {
    // 検索データからコピーすべき元データを抽出
    const selectedEntity: KanrenshaKigyouDtMasterEntityInterface | undefined = resultDto.value.listMasterKigyouDt.filter((e) => e.kanrenshaKigyouDtMasterId === selectedNo)[0];
    if (undefined !== selectedEntity) {
        emits("sendKigyouDtInterface", selectedEntity);
    } else {
        infoLevel.value = MessageConstants.LEVEL_ERROR;
        messageType.value = MessageConstants.VIEW_OK;
        //title.value = MESS_PAGE_NAME;
        message.value = getErrorUniqueIdMessage(selectedNo);
    }
}

function onCancel() {
    emits("sendCancelKigyouDt");
}

function recievePagingNumber(selecteddNumber: number) {
    pageNumber.value = selecteddNumber;
    onSearch();
}

function recieveSubmit() {
    // メッセージ表示後の分岐はない

    // 非表示
    infoLevel.value = 0;
    messageType.value = 0;
}
</script>
<template>
    <h3>関連者企業団体検索</h3><br>

    <div class="one-line">
        <div class="left-area">
            法人番号(前方一致)
        </div>
        <div class="right-area">
            <input type="text" v-model="capsuleDto.houjinNo" class="name-input">
        </div>
    </div>

    <div class="one-line">
        <div class="left-area">
            名前(前方一致)
        </div>
        <div class="right-area">
            <input type="text" v-model="capsuleDto.name" class="name-input">
        </div>
    </div>

    <div class="one-line">
        <div class="left-area">
            住所(前方一致)
        </div>
        <div class="right-area">
            <input type="text" v-model="capsuleDto.address" class="max-input">
        </div>
    </div>

    <div class="one-line">
        <div class="left-area">
            代表者
        </div>
        <div class="right-area">
            <input type="text" v-model="capsuleDto.delegate" class="name-input">
        </div>
    </div>

    <div class="one-line">
        <div class="left-area">
            検索
        </div>
        <div class="right-area">
            <button @click="onSearch">検索</button>
        </div>
    </div>

    <h3>検索結果</h3><br>

    <!-- ページング -->
    <PagingControl :all-count="allCount" :limit="limit" :page-number="pageNumber"
        @send-paging-number="recievePagingNumber"></PagingControl>

    <div class="one-line">
        <table>
            <tbody>
                <tr>
                    <th>関連者コード(企業／団体)</th>
                    <th>法人番号</th>
                    <th>法人名</th>
                    <th>住所</th>
                    <th>代表者</th>
                    <th>&nbsp;</th>
                </tr>
                <tr v-for="entity of resultDto.listMasterKigyouDt" :key="entity.kanrenshaKigyouDtMasterId">
                    <td>{{ entity.kigyouDtKanrenshaCode }}</td>
                    <td>{{ entity.houjinNo }}</td>
                    <td>{{ entity.kanrenshaName }}</td>
                    <td>{{ entity.allAddress }}</td>
                    <td>{{ entity.kigyouDtDelegate }}</td>
                    <td><button @click="onSelectRow(entity.kanrenshaKigyouDtMasterId)">選択</button></td>
                </tr>
            </tbody>
        </table>
    </div>
    <!-- ページング -->
    <PagingControl :all-count="allCount" :limit="limit" :page-number="pageNumber"
        @send-paging-number="recievePagingNumber"></PagingControl>

    <div class="footer" v-if="props.isRaiseCommponet">
        <button @click="onCancel" class="footer-button">キャンセル</button>
    </div>

    <!-- メッセージ -->
    <div class="overMessage" v-if="messageType !== MessageConstants.VIEW_NONE">
        <MessageView :info-level="infoLevel" :message-type="messageType" :title="MESS_PAGE_NAME" :message="message"
            :caller="caller" @send-submit="recieveSubmit">
        </MessageView>
    </div>

</template>
<style scoped>
table {
    border-style: solid;
    border-width: 1px;
}

td {
    border-style: solid;
    border-width: 1px;
}

th {
    border-style: solid;
    border-width: 1px;
}
</style>
